...
public class Demohandler {
	...
	public String validateDetails(UserBean userBean, MessageContext messageContext) {
		...
		/*if (userBean.getConformpassword().isEmpty()) {
			messageContext.addMessage(
					new MessageBuilder().error().source("password").defaultText("Password cannot be Empty").build());
			status = "failure";
		}*/
		if (userBean.getPassword().isEmpty()) {
			messageContext.addMessage(new MessageBuilder().error().source("conformpassword")
					.defaultText("ConformPassword cannot be Empty").build());
			status = "failure";
		}
	...
	public void sendMail(UserBean usb) {
		String toAddress = usb.getEmail();
		String fromAddress = "muthucse4038@gmail.com";
		String subject = "MusicWorld";
		String body = "Welcome to the empire of MUSIC WORLD!!! Load your ears with musical songs... Happy shopping folks...";
